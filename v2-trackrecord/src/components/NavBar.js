...
function NavBar({ onMenuToggle, menuActive }) {
  ...
  return (
    <nav className={`nav ${menuActive ? "nav-expanded" : "nav-collapsed"}`}>
      <div
        className={`hamburger-menu ${menuActive ? "hide-menu" : "show-menu"}`}
        onClick={toggleMenu}
      >
        <div className={menuActive ? "hamburger-bar" : "hamburger-bar2"}></div>
        <div className={menuActive ? "hamburger-bar" : "hamburger-bar2"}></div>
        <div className={menuActive ? "hamburger-bar" : "hamburger-bar2"}></div>
      </div>

      <div className={`logo ${menuActive ? "logo-show" : "logo-hide"}`}>
        <Link to="/">
